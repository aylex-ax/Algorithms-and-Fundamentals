...
An extension of the base [Doubly Linked List](../Project%20-%2001%20-%20Doubly-Linked-List) project. This update adds automatic size tracking to the list, so the number of nodes can be retrieved in constant time without traversing the entire list.
...
2. Five values (`5, 4, 3, 2, 1`) are inserted at the beginning, resulting in the list `1 2 3 4 5`.
3. The list is printed to show its current content.
4. `Size()` is called and its result is printed, confirming the node count is tracked correctly.
...
3. Build and run — the program will print the list content followed by its size.
